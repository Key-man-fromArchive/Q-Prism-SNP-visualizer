...
- Include decimal comma, semicolon delimiter, malformed well, duplicate `(well, cycle, channel)`, missing required role, and missing normalization channel cases.
...
- Verification: each downloadable template has a matching strict parser test.
...
- Verification: invalid large/malformed inputs fail before session creation.

## Phase P3: Preview APIs & Mapping Workflow

### P3-R1-T1: Implement `POST /api/import/preview`

- Return owner-bound `preview_id`, table/sheet candidates, inferred delimiter/header/data rows, channel candidates, sample rows, and warnings.
...
- Verification: API tests cover success, recoverable validation errors, expired preview ids, and unauthorized preview access.
...
- Build wizard steps: upload preview, worksheet/table selection, header/data row confirmation, format selection, file structure mapping, assay role mapping, validation preview, import.
- Separate channel detection from assay role binding.
- Allow returning to mapping without re-uploading.
- Verification: Playwright covers a successful WT/MT + normalization import and a recoverable missing-role error.
...
- P5 role-aware visualization should not block strict WT/MT duplex import, but it is required for full triplex/quadruplex support.
