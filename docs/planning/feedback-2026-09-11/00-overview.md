...
| ~~D-5~~ | ~~탭 ID 전면 변경~~ | **해결 (2026-09-11)** — 전면 재편 채택. 실측 영향 약 48곳(파일 9개)으로 기계적 치환 수준 | — |
| ~~D-6~~ | ~~산점도 종횡비~~ | **해결 (2026-09-11)** — 고정하지 않고 드롭다운으로 `4:3`(기본)/`1:1` 선택 | — |
...
- ~~**종횡비 목표(FB-04)**~~ → 해결: 고정 대신 사용자 선택(드롭다운)으로 양쪽 모두 지원한다.
