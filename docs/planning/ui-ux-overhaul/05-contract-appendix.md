...
Well role and marker membership are independent: outside-marker rows retain meaningful captured controls/manual labels (including NTC, Empty/Omit and explicit manual Unknown), but always carry `assignment_status=outside_marker`; an ordinary outside-marker sample with no such override is Unassigned. Do not invent or apply a marker genotype outside its captured membership. Reference labeling prefers captured `normalization_dye`, then explicit `normalization_channel`; use legacy ROX only when no more specific channel is known, otherwise report an unknown reference rather than guessing.
