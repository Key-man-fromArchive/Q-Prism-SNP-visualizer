...
```
- 업로드 시: 파일에 명시적 Target/Assay가 있으면 해당 마커와 웰 배정을 복원한다. 없으면 마커를 자동 생성하지 않고 사용자가 Plate 탭에서 정의한다.
...
| # | 질문 | 결정 | 근거 |
|---|---|---|---|
| Q1 | Plate Setup 필수 게이트? | **비차단(opt-in)**. 명시적 파일 메타데이터는 복원하고, 없으면 빈 Plate Setup으로 시작 | 일부 PCRD/EDS는 Target/Assay를 제공하지만 일반 well-group은 마커 근거가 아님 |
...
- **Q1 강도**: 비차단은 유지한다. 단, 파일이 제공하는 명시적 Target/Assay는 복원하며 CFX `wellGroup` 같은 일반 분석 subset은 절대 마커로 승격하지 않는다.
...
- **마커는 기본 0개 · 직접 추가**: 단, PCRD/EDS에 명시적 Target/Assay와 웰 배정이 있으면 그것만 자동 복원한다. CFX `Group 1..12` 등 일반 well-group은 자동 마커 금지.
