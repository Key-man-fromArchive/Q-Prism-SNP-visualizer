...
const ko: Translations = {
  ...
  // P29-CALENDAR: 세션 목록의 달력 보기 (기본 보기는 그대로 표 형태)
  sessionViewToggleLabel: '세션 보기 방식',
  sessionViewTable: '표',
  sessionViewCalendar: '달력',
  calendarAllSessions: (n: number) => `전체 ${n}개 세션`,
  calendarActiveDays: (n: number) => `작업한 날 ${n}일`,
  calendarOpen: '열기',
  calendarDensityLabel: '일별 세션',
  calendarDensityLow: '1–2건',
  calendarDensityMedium: '3–5건',
  calendarDensityHigh: '6건 이상',
  calendarPrevMonth: '이전 달',
  calendarNextMonth: '다음 달',
  calendarToday: '오늘',
  calendarGridLabel: '세션 달력',
  calendarWeekdayShort: (day: number) => (['일', '월', '화', '수', '목', '금', '토'][day] ?? ''),
  calendarMonthLabel: (year: number, month: number) => `${year}년 ${month + 1}월`,
  calendarDateLabel: (year: number, month: number, day: number) => `${year}년 ${month + 1}월 ${day}일`,
  calendarDayHasSessions: (n: number) => `${n}개 세션`,
  calendarNoSessionsMonth: '이 달에는 세션이 없습니다.',
  calendarNoDateSelected: '날짜를 선택하면 그날의 세션이 표시됩니다.',
  calendarNoSessionsOnDate: '이 날짜에는 세션이 없습니다.',
  addSelectedToProject: (n: number) => `선택한 ${n}개를 프로젝트에 추가`,
  deleteSelected: (n: number) => `선택한 ${n}개 삭제`,
  session: '세션',
  instrument: '장비',
  uploaded: '업로드 일시',
  rawFileDownload: '원본 파일 다운로드',
  rawFileDownloadedTitle: (name: string, expiresOn: string) => `${name} 다운로드 (${expiresOn}에 만료)`,
  rawFileExpiredTitle: (expiredOn: string) => `원본 파일이 ${expiredOn}에 만료되어 삭제되었습니다. 판독값과 판정 결과는 영향을 받지 않습니다.`,
  rawFileMissingTitle: '원본 파일을 저장소에서 찾을 수 없습니다 (예상치 못한 문제). 판독값과 판정 결과는 영향을 받지 않습니다. 관리자에게 문의하세요.',
  rawFileDownloadFailed: '원본 파일을 다운로드하지 못했습니다. 방금 만료되었을 수 있습니다 -- 새로고침 후 다시 시도하세요.',
