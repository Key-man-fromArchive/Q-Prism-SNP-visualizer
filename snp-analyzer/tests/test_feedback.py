...
# ---------------------------------------------------------------------------
# Migration 8 (on a simulated v6 DB)
# ---------------------------------------------------------------------------


def test_migration_8_adds_feedback_tables(tmp_path):
    import sqlite3
    ...
    # The feedback tables are migration 8; 7 is sessions.input_revision, which
    # init_db() applies on the way past. Assert 8 is recorded rather than that
    # it is the newest, so a later migration does not break this test.
    versions = {r[0] for r in conn.execute("SELECT version FROM schema_version").fetchall()}
    assert 8 in versions

    # Migration 8 back-fills nothing.
